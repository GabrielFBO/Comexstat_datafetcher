...
while True:
    ...
    if save.lower() == 'y':
        ...
        if format.capitalize() == 'csv':
            df_country.to_csv(f'results/{filename}_imp_2025.csv', index=False)
            print('CSV file saved!')
        elif format.title() == 'excel':
            df_country.to_excel(f'results/{filename}_imp_2025.xlsx', index=False)
            print('Excel sheet saved!')
        elif format.capitalize() == 'json':
            df_country.to_excel(f'results/{filename}_imp_2025.json', index=False)
